...
void computeMandelbrotPointsMPI(int width, int height, double xmin, double xmax, 
...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        if (rank == 0) {
            std::cerr << "Usage: mpirun -np <num_processes> " << argv[0] << " <max_iter> <processes>" << std::endl;
        }
        MPI_Finalize();
        return 1;
    }
    ...
    // Only for naming the output image
    ...
    // Compute local mandelbrot points
    auto start_time = std::chrono::high_resolution_clock::now();
    computeMandelbrotPointsMPI(width, height, xmin, xmax, ymin, ymax, 
    ...
    if (rank == 0) {
        std::string output_image = "mandelbrot_mpi_" + std::to_string(max_iter) + "_processes_" + std::to_string(processes) + ".png";
